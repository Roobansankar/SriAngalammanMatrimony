...
export default function Step4({ nextStep, prevStep, formData }) {
  ...
  useEffect(() => {
    async function fetchOptions() {
      try {
        ...
        setOptions((prev) => ({
          ...prev,
          moonSigns: moonData.map((m) => ({ id: m.ID, name: m.Moon_Sign })),
          gothras: gothraData.map((g) => g.Gothra),
          mangliks: manglikData.map((m) => m.type),
          shanis: shaniData.map((s) => s.type),
          horoscopeMatches: matchData.map((h) => h.type),
  ...
  // Fetch nakshatras when moon sign changes
  useEffect(() => {
    async function fetchNakshatras() {
      ...
      try {
        // Find the moon sign ID
        const moonSignObj = options.moonSigns.find(
          (m) => m.name === data.moonSign
        );
        if (!moonSignObj) return;

        const response = await fetch(
          `${API_BASE}nakshatra/${moonSignObj.id}`
        );
        const nakshData = await response.json();

        setOptions((prev) => ({
          ...prev,
          nakshatras: nakshData.map((n) => n.nakshatra_paatham),
        }));
      } catch (err) {
        console.error("Failed to fetch nakshatras:", err);
      }
    }

    fetchNakshatras();
  }, [data.moonSign, options.moonSigns]);
  ...
  const handleChange = (e) => {
    ...
    if (files) {
      ...
      // Reset star when moon sign changes
      if (name === "moonSign") {
        setData({ ...data, [name]: value, star: "" });
      } else if (
        name === "placeOfShani" ||
        name === "placeOfBirth" ||
        name === "countryOfBirth"
...
const handleNext = () => {
  ...
  return (
    <div className="bg-white shadow-lg rounded-2xl w-full max-w-2xl p-6 mx-auto border border-yellow-200 mt-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            value={data.star}
            onChange={handleChange}
            disabled={!data.moonSign}
            className="border p-2 rounded-lg w-full focus:ring-2 focus:ring-yellow-500 outline-none disabled:bg-gray-100 disabled:cursor-not-allowed"
          >
            <option value="">
              {data.moonSign ? "Select Star" : "Select Moon Sign first"}
            </option>
            {options.nakshatras.map((s) => (
              <option key={s} value={s}>
                {s}
      ...
      <h2 className="text-lg font-bold text-yellow-700 mb-2 mt-6">
        12 Rasi Boxes (ராசி)
      </h2>
      ...
      <label className="flex items-center justify-center border-2 border-dashed border-yellow-400 rounded-lg py-3 cursor-pointer mt-6">
        <FileUp className="w-5 h-5 text-yellow-600 mr-2" />
