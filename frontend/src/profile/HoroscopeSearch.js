...
//             : [],
//           // use the 'status' field returned by your /api/star route
//           star: Array.isArray(star) ? star.map((s) => s.status) : [],
//           maritalstatus: Array.isArray(maritalstatus)
//             ? [...new Set(maritalstatus.map((m) => m.status).filter(Boolean))]
//             : [],
//           castes: [],
//         });
//         console.log("star API response:", star); // optional: debug
//       } catch (err) {
...
export default function HoroscopeSearch() {
  ...
  const [form, setForm] = useState({
    ...
    religion: [],
    caste: [],
    star: [], // multiple star selections (strings)
    Sevai: "0",
    Raghu: "0",
    Keethu: "0",
    edu: [],
  ...
  useEffect(() => {
    async function fetchOptions() {
      try {
        ...
        const occupations = await occuRes.json();
        const maritalstatus = await marRes.json();
        const starApi = await starRes.json();

        // Add your static star options here and merge with API-provided stars
        const staticStars = ["Sevai", "Raghu", "Keethu"];
        const apiStars =
          Array.isArray(starApi) && starApi.length
            ? starApi.map((s) => (typeof s === "string" ? s : s.status || ""))
            : [];

        const combinedStars = Array.from(
          new Set([...staticStars, ...apiStars].filter(Boolean))
        );
        ...
        setOptions({
          ...
          occupations: Array.isArray(occupations)
            ? occupations.map((o) => o.occu)
            : [],
          // merged star list (includes Sevai, Raghu, Keethu)
          star: combinedStars,
          maritalstatus: Array.isArray(maritalstatus)
            ? [...new Set(maritalstatus.map((m) => m.status).filter(Boolean))]
            : [],
        ...
        console.log("star API response:", starApi); // optional: debug
      } catch (err) {
        console.error("Failed to load options:", err);
      }
    }
  ...
  function submitSearch(e) {
    e.preventDefault();
    navigate("/horoscopesearch-results", {
      state: { filters: form, apiBase: API_BASE },
    });
  }
