...
// import { connectSocket, getSocket } from "../socket"; // adjust path if necessary
// import { Link } from "react-router-dom";

// /**
//  * Recipient Interests Page
//  *
//  * Expects these backend endpoints:
//  * - GET  /api/auth/interest/incoming?to=<matriid>   (recommended)
//  * - POST /api/auth/interest/respond  { interestId, action }  // action = 'accepted'|'rejected'
//  * - GET  /api/auth/searchByMatriID?matriid=<matriid>        // to fetch sender profile / contact
//  *
//  * If you don't have /interest/incoming, the page will still work using real-time socket events,
//  * but you should implement the incoming endpoint for initial load.
//  */
...
//       try {
//         // recommended API endpoint: GET /api/auth/interest/incoming?to=<matriid>
...
//         }
//       } catch (e) {
//         // If backend doesn't have incoming route, we still continue — rely on socket events
//         console.warn(
...
//   // socket: listen for interest_received events (real-time)
//   useEffect(() => {
//     connectSocket();
...
//     // Also listen for updates (when someone accepted/rejected)
...
//     socket?.on("interest_response", onUpdate); // sometimes responses may be routed here

//     return () => {
//       socket?.off("interest_received", onReceived);
//       socket?.off("interest_update", onUpdate);
//       socket?.off("interest_response", onUpdate);
//     };
...
//   // helper to render contact (unblur when accepted)
//   const renderContact = (item) => {
//     if (item.status === "accepted") {
//       // prefer senderProfile (fetched) but fall back to a shallow contact object if present
//       const profile = item.senderProfile;
//       const phone =
//         profile?.Mobile || profile?.Phone || item.senderMobile || "N/A";
//       const email =
//         profile?.ConfirmEmail || profile?.email || item.senderEmail || "N/A";
//       return (
...
//     // if pending or rejected - show blurred contact hint
//     return (
//       <div className="text-sm text-gray-500">
//         Contact hidden until you accept
//       </div>
//     );
//   };
...
//           <h1 className="text-2xl font-bold">Incoming Interests</h1>
...
//               No incoming interests yet. When someone sends interest, you'll see
